    ...
    public class User
    {
        ...
        public MethodReturn LiftUser()
        {
            ...
            if (mr.IsSuccess)
            {
                if (UserExist(Username))
                {
                    mr.IsSuccess = dbcon.Exec("UPDATE tbl_users set islocked='0' WHERE uid='" + UID + "'");
                    mr.Message = mr.IsSuccess ? "Successfully lifted user [ " + Username + " ]!" : "Something went wrong please, try again!";
                }
                else
                {
                    mr.IsSuccess = false;
                    mr.Message = "Username does not exist!";
                }
            }
            else
            {
